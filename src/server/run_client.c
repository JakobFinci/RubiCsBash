...
int main(void) {
  ...
  // Connect to the server running on the ip ADDRESS.
  struct sockaddr_in server_addr = socket_address(inet_addr(ADDRESS), PORT);
  ...
  while (socket_file_status != -1) {
    socket_file_status = run_cube(socket_descriptor);
  }
